<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::table('researches', function (Blueprint $table) {
            // Leader information
            $table->string('nidn_leader')->nullable()->after('file_final_report');
            $table->string('leader_name')->nullable()->after('nidn_leader');

            // Institution information
            $table->string('pddikti_code_pt')->nullable()->after('leader_name');
            $table->string('institution')->nullable()->after('pddikti_code_pt');

            // Skema information
            $table->string('skema_abbreviation')->nullable()->after('institution');
            $table->string('skema_name')->nullable()->after('skema_abbreviation');

            // Year information
            $table->year('the_first_year_of_the_proposal')->nullable()->after('skema_name');
            $table->year('proposed_year_of_activities')->nullable()->after('the_first_year_of_the_proposal');
            $table->year('the_year_of_the_activity')->nullable()->after('proposed_year_of_activities');

            // Activity duration
            $table->integer('duration_of_activity')->nullable()->after('the_year_of_the_activity');

            // Proposal status
            $table->enum('proposal_status', [
                'draft',
                'submitted',
                'approved',
                'rejected',
                'revision',
                'funded',
                'completed'
            ])->default('draft')->after('duration_of_activity');

            // Funding information
            $table->decimal('funds_are_approved', 15, 2)->nullable()->after('proposal_status');
            $table->string('sinta_afiliasi_id')->nullable()->after('funds_are_approved');
            $table->string('funds_institution')->nullable()->after('sinta_afiliasi_id');

            // Target and program information
            $table->integer('target_tkt_level')->nullable()->after('funds_institution');
            $table->string('hibah_program')->nullable()->after('target_tkt_level');
            $table->string('focus_area')->nullable()->after('hibah_program');

            // Fund source information
            $table->string('fund_source_category')->nullable()->after('focus_area');
            $table->string('fund_source')->nullable()->after('fund_source_category');
            $table->string('country_fund_source')->default('Indonesia')->after('fund_source');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::table('researches', function (Blueprint $table) {
            $table->dropColumn([
                'nidn_leader',
                'leader_name',
                'pddikti_code_pt',
                'institution',
                'skema_abbreviation',
                'skema_name',
                'the_first_year_of_the_proposal',
                'proposed_year_of_activities',
                'the_year_of_the_activity',
                'duration_of_activity',
                'proposal_status',
                'funds_are_approved',
                'sinta_afiliasi_id',
                'funds_institution',
                'target_tkt_level',
                'hibah_program',
                'focus_area',
                'fund_source_category',
                'fund_source',
                'country_fund_source'
            ]);
        });
    }
};
